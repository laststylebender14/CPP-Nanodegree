...
void TrafficLight::waitForGreen()
{
    ...
    while (true)
    {
        ...
        TrafficLightPhase cur = _messageQueue.receive();
...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::mt19937 m(rd());
    std::uniform_real_distribution<> dist(4000.0, 6000.0);
    double cycledur = dist(m); 
    auto lst_upd = std::chrono::system_clock::now();
    ...
    while (true)
    {
        ...
        long timeSinceLastUpdate = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lst_upd).count();
        if (timeSinceLastUpdate >= cycledur)
        {
            ...
            lst_upd = std::chrono::system_clock::now();
